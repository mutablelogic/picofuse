...
test_main_sys(0) {
...

  {
    ...
    test_assert(sys_string_compare(sys_env_arg_string(args, 0), "in.txt") ==
                0);
...

  {
    ...
    test_assert(args != NULL);
    test_assert(sys_env_arg_count(args) == 2);
    test_assert(sys_string_compare(sys_env_arg_string(args, 0),
                                    "--not-a-flag") == 0);
...

  {
    ...
    char namebuf[16] = {0};
    test_assert(sys_env_arg_parse_string(args, "name", namebuf,
                                          sizeof(namebuf)) == 3);
    int32_t count = 0;
    test_assert(sys_env_arg_parse_int32(args, "count", &count));
    test_assert(count == 3);
...

  {
    ...
    sys_init(SYS_ENV_ARG_CAPACITY + 2, argv, 0);
