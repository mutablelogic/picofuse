...
// (unlike a host build, which already has one) - factored out of
// net_010/main.c, the first test to need it. Requires WIFI_SSID/
// WIFI_PASSWORD as compile definitions (see test/CMakeLists.txt).
 ...
 * @return Connected handle, or NULL if WIFI_SSID is unset or there's no
 * Wi-Fi backend on this board (printed, not asserted - those are "not
 ...
 * on NULL (i.e. test_wifi_join() itself skipped or failed softly). */
