...
void _pix_bitmap_rgb888_set_pixel(pix_bitmap_t *bitmap, pix_point_t point,
  ...
  if (bitmap->op == PIX_BLEND) {
    ...
    if (a != 255) {
      // RGB888 stores no alpha of its own - treat the existing pixel as
      // fully opaque for the blend (PIX_COLOR_RGB() does exactly that).
      color = pix_color_blend(color, PIX_COLOR_RGB(p[0], p[1], p[2]));
    }
  }
...
void _pix_bitmap_rgb888_fill_rect(pix_bitmap_t *bitmap, pix_point_t origin,
  ...
  if (bitmap->op == PIX_SET || a == 255) {
    ...
    // A grayscale color (R==G==B) - every byte in the fill is identical,
    // so memset() can fill a whole row in one call instead of one pixel at
    // a time.
    if (r == g && g == b) {
  ...
  for (uint16_t dy = 0; dy < size.h; dy++) {
    uint8_t *p = (uint8_t *)bitmap->data +
                ...
                (size_t)origin.x * 3;
    for (uint16_t dx = 0; dx < size.w; dx++) {
      pix_color_t blended = pix_color_blend(color, PIX_COLOR_RGB(p[0], p[1], p[2]));
