...
// (mutex, condition variable, wait-group). RP2040/RP2350 only have a
// handful of hardware spin locks, and each pool only ever holds this lock
// for a few array-bookkeeping instructions, so sharing one lock across all
// of them costs far less than dedicating a spin lock to each.
