...
typedef struct {
  ...
  uint32_t timeout;      // seconds to wait before killing a hung run, or 0
                          // to wait indefinitely (no timeout)
...
// bounds flashing and marker collection together, or (if 0) doesn't bound
// them at all. Returns false on setup, OpenOCD, or marker failure,
// including a "[PANIC] " line or timeout.
